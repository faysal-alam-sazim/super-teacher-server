...
export class Migration20240823141502_updated_user extends Migration {

  async up(): Promise<void> {
    this.addSql('create table "unique_code" ("id" serial primary key, "created_at" timestamptz not null, "updated_at" timestamptz not null, "email" varchar(255) not null, "code" varchar(255) not null, "reset_counter" int not null);');
    this.addSql('alter table "unique_code" add constraint "unique_code_email_unique" unique ("email");');
    this.addSql('alter table "unique_code" add constraint "unique_code_code_unique" unique ("code");');

    this.addSql('create table "users" ("id" serial primary key, "created_at" timestamptz not null, "updated_at" timestamptz not null, "first_name" varchar(255) not null, "last_name" varchar(255) not null, "gender" text check ("gender" in (\'Male\', \'Female\')) not null, "email" varchar(255) not null, "password" varchar(255) not null, "role" text check ("role" in (\'Student\', \'Teacher\')) not null);');
